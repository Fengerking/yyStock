...
public class MyStockActivity extends AppCompatActivity {
    ...
    private void showResult () {
        ...
        for (i = 0; i < m_lstStockComp.size(); i++) {
            ...
            stkView.m_strItem = String.format("%s %,.2f %,.0f %,.0f", stkComp.m_strName, dCompValue, stkComp.m_dCash, stkComp.m_dDebt);
            stkView.m_nTextColor = Color.rgb(255, 255,255);
            stkView.m_nTextSize = 16;
            ...
            for (j = 0; j < m_lstStockInfo.size(); j++) {
                ...
                if (stkItem.m_strComp.compareTo(stkComp.m_strComp) != 0)
                    continue;
                stkView = new stockView ();
                stkView.m_strItem = String.format("%s %,.2f %,.1f%% %,.1f %,.1f", stkItem.m_strName, stkItem.m_dNowPrice * stkItem.m_nNumber,
        ...
        for (j = 0; j < m_lstStockInfo.size(); j++) {
            ...
            if (strItem.compareTo(stkItem.m_strCode) == 0) {
                ...
                if (nTotalNum > 0) {
                    stkView = new stockView ();
                    stkView.m_strItem = String.format("%s %,.1f %,.1f %,.1f%%", strName,
        ...
        for (i = 0; i < lstItems.size(); i++) {
            ...
            stkView.m_strItem = String.format("%s %d %,.1f %,.1f%%, %,.1f", stkItem.m_strName,
        ...
        m_lstResult.add (new stockView ());
        stkView = new stockView ();
        stkView.m_strItem = String.format("汇总：%,.2f %,.0f %,.0f", dTotalStock, dTotalCash, dTotalDebt);
        stkView.m_nTextSize = 16;
        m_lstResult.add (stkView);
        stkView = new stockView ();
        stkView.m_strItem = String.format("资产：%,.2f %,.0f", dTotalStock + dTotalCash - dTotalDebt, dTotalWin);
        stkView.m_nTextSize = 16;
        m_lstResult.add (stkView);
